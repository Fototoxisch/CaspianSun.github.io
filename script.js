...
async function sendDatabaseAlert(botToken, chatId, errorMessage) {
    ...
    try { await fetch(url); } catch (error) { console.error('Ошибка алерта:', error); }
}

function showDbError(msg) {
    if(document.getElementById('db-error-banner')) return;
    const banner = document.createElement('div'); banner.id = 'db-error-banner';
    ...
    banner.innerHTML = `⚠️ Ошибка базы данных (${msg}). Вероятно, проект Supabase уснул. Зайдите в панель Supabase для восстановления. <button onclick="this.parentElement.remove()" style="margin-left:15px;background:none;border:1px solid #fff;color:#fff;padding:2px 8px;cursor:pointer;border-radius:4px;">Закрыть</button>`;
    document.body.prepend(banner);
    if (dbSettings?.tg_token && dbSettings?.tg_chat_ids?.length > 0) {
        dbSettings.tg_chat_ids.forEach(chat => { sendDatabaseAlert(dbSettings.tg_token, chat.id, msg); });
    }
}
...
async function mutateDB(table, method, bodyData, id = null) { 
    ...
    if (res.status === 401) { alert("Сессия истекла."); sessionStorage.removeItem('supabase_admin_token'); window.location.reload(); throw new Error('Сессия истекла'); }
    if (!res.ok) throw new Error('Ошибка БД'); 
}

// Защита прямых переходов (отработка скрытого товара)
function protectProductPage() {
...
async function syncProductsWithCatalog() {
    try {
        ...
        const cards = doc.querySelectorAll('.tech-card[data-slug]');
        ...
        cards.forEach(card => {
            ...
            if(!dbProducts.find(p => p.slug === slug)) {
        ...
        for (let prod of newProducts) { await mutateDB('products', 'POST', prod); }
        
        let orphaned = dbProducts.filter(p => !catalogSlugs.includes(p.slug));
        for (let orphan of orphaned) { await mutateDB('products', 'DELETE', null, orphan.id); }
        
        if(newProducts.length > 0 || orphaned.length > 0) { dbProducts = await fetchDB('products'); }
    } catch (e) { console.error("Ошибка автосинхронизации:", e); }
}
...
// Скрытие позиций из каталога
function syncCatalogVisibility() {
    const cards = document.querySelectorAll('.tech-card[data-slug]');
    if(!cards.length) return;
    cards.forEach(card => {
        const slug = card.getAttribute('data-slug');
        const dbItem = dbProducts.find(p => p.slug === slug);
        if(dbItem && dbItem.published === false) {
...
if (authForm) { 
    authForm.addEventListener('submit', async (e) => { 
        ...
        try { 
            ...
            const data = await res.json(); 
            if (res.ok && data.access_token) { sessionStorage.setItem('supabase_admin_token', data.access_token); window.location.reload(); } 
            else { err.innerText = (res.status === 400) ? 'Неверно' : (data.error_description || 'Ошибка'); err.classList.remove('hidden'); } 
...
window.togglePub = async function(id, currentState) {
    try {
        await mutateDB('products', 'PATCH', { published: !currentState }, id);
        dbProducts = await fetchDB('products');
        renderAdminLists();
        showAdminSuccess();
...
function renderAdminLists() { 
    ...
    if (prodList) { 
        prodList.innerHTML = dbProducts.map((p) => { 
            let isPub = p.published !== false;
            let badgeClass = isPub ? 'active' : '';
            let badgeText = isPub ? 'Опубликовано' : 'Скрыто';
            return `<div class="card-base" style="display:flex; flex-direction:row; align-items:center; gap:16px; padding: 16px;">
                ${p.image?`<img src="${p.image}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;">`:'<div style="width:60px;height:60px;background:#000;display:flex;align-items:center;justify-content:center;border-radius:8px;font-size:20px;">📸</div>'}
                <div style="flex:1;">
                    <h3 style="margin:0; font-size:16px; font-family:var(--font-tech);">${p.title || p.slug}</h3>
                    <div style="color:var(--text-muted); font-size:12px; margin-top:4px;">${p.price || 'Цена не указана'} | /products/${p.slug}.html</div>
                </div>
...
function applyAdvancedFilters() { 
    ...
    cards.forEach(card => { 
        if (card.classList.contains('force-hidden')) return; 
...
document.addEventListener('click', (e) => { 
    ...
    if (btnLead) { document.getElementById('lead-hidden-service').value = btnLead.getAttribute('data-service'); document.getElementById('lead-service-name').innerText = `Услуга: ${btnLead.getAttribute('data-service')}`; document.getElementById('lead-modal')?.classList.add('active'); document.body.style.overflow = 'hidden'; } 
});
...
if (tgForm) { 
    tgForm.addEventListener('submit', function(e) { 
        e.preventDefault(); const token = dbSettings.tg_token; const chatIds = dbSettings.tg_chat_ids || []; 
        if (!token || chatIds.length === 0) { alert("Настройте токен и Chat ID в панели администратора!"); return; } 
        const btn = document.getElementById('lead-submit-btn'); const name = document.getElementById('lead-name').value.trim(); const phone = document.getElementById('lead-phone').value.trim(); const comment = document.getElementById('lead-comment') ? document.getElementById('lead-comment').value.trim() : ''; const service = document.getElementById('lead-hidden-service').value; 
        if(name === '.' || name === '/start' || phone === '.' || phone === '/start' || comment === '.' || comment === '/start') { document.getElementById('lead-success').innerText = 'Система заблокировала пустой запрос'; document.getElementById('lead-success').classList.remove('hidden'); setTimeout(() => {document.getElementById('lead-success').classList.add('hidden'); document.getElementById('lead-modal').classList.remove('active'); document.body.style.overflow = ''; }, 2000); return; } 
