// =========================================================
// 🌐 SUPABASE API (УЛЬТРА-ОПТИМИЗИРОВАНО)
// =========================================================
...
let dbProducts = [], dbServices = [], dbPortfolio = [];
...
async function mutateDB(table, method, bodyData, id = null) {
    ...
    if (!res.ok) throw new Error('Ошибка базы данных. Проверьте права доступа.');
}
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok && data.access_token) {
                ...
                err.innerText = (res.status === 400) ? 'Неверная почта или пароль' : (data.error_description || 'Ошибка авторизации');
                err.classList.remove('hidden');
            }
        } catch (error) { err.innerText = 'Ошибка соединения'; err.classList.remove('hidden'); }
        btn.innerText = 'Войти в систему';
    });
}
if (document.getElementById('btn-logout')) {
    document.getElementById('btn-logout').addEventListener('click', () => { sessionStorage.removeItem('supabase_admin_token'); window.location.reload(); });
}
...
function processImage(file, previewId, hiddenInputId, textId) {
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            document.getElementById(previewId).src = b64; document.getElementById(previewId).classList.remove('hidden');
            document.getElementById(hiddenInputId).value = b64; document.getElementById(textId).classList.add('hidden');
        };
    };
}
...
function handleFilterChange(sel) {
    if (sel.id === 'custom-category' && document.getElementById('catalog-grid')) {
        ...
        document.querySelectorAll('.specific-filter').forEach(group => { group.classList.add('hidden'); group.querySelector('.custom-select').setAttribute('data-value', 'all'); });
        ...
        const val = sel.getAttribute('data-value');
        document.querySelectorAll('.panels-only').forEach(el => { if(val === 'panels') el.classList.remove('hidden'); else el.classList.add('hidden'); });
        document.querySelectorAll('.inv-bat-only').forEach(el => { if(val === 'inverters' || val === 'batteries') el.classList.remove('hidden'); else el.classList.add('hidden'); });
        document.querySelectorAll('.inv-only').forEach(el => { if(val === 'inverters') el.classList.remove('hidden'); else el.classList.add('hidden'); });
        document.querySelectorAll('.bat-only').forEach(el => { if(val === 'batteries') el.classList.remove('hidden'); else el.classList.add('hidden'); });
    }
}
...
// =========================================================
// ⚙️ АДМИНКА (УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК ФОРМ)
// =========================================================
...
function renderAdminLists() {
  ...
  if (prodList) {
    prodList.innerHTML = dbProducts.map((p, idx) => {
      let specs = p.specs_card ? p.specs_card.map(s => `<div><span>${s.label}</span> <strong>${s.value}</strong></div>`).join('') : '';
      return `<div class="tech-card card-base card-item" data-id="${idx}">${p.image?`<img src="${p.image}" class="product-image-rendered" loading="lazy">`:''}<h3>${p.title}</h3><p class="card-sub">${p.subtitle}</p><div class="card-specs">${specs}</div><div class="card-footer"><span class="price">${p.price}</span><div class="card-actions"><button type="button" class="btn btn-sm btn-drawer flex-2">👀 Вид</button><button type="button" class="btn btn-sm btn-edit flex-2" onclick="editItem('products',${idx})">Ред.</button><button type="button" class="btn btn-sm btn-delete flex-2" onclick="deleteItem('products',${idx})">Удал.</button></div></div></div>`;
    }).join('');
  }
  ...
  if (portList) {
    portList.innerHTML = dbPortfolio.map((p, idx) => {
      let img = p.images?`<img src="${p.images}" class="port-img" loading="lazy">`:`<div class="mock-img port-img">📸</div>`;
      return `<div class="portfolio-card card-base card-item" data-id="${idx}">${img}<div class="port-content"><h3 class="port-title">${p.title}</h3><p class="port-desc">${p.desc_text}</p><div class="card-actions"><button type="button" class="btn btn-sm btn-drawer-port flex-2">👀 Вид</button><button type="button" class="btn btn-sm btn-edit flex-2" onclick="editItem('portfolio',${idx})">Ред.</button><button type="button" class="btn btn-sm btn-delete flex-2" onclick="deleteItem('portfolio',${idx})">Удал.</button></div></div></div>`;
    }).join('');
  }
}

window.deleteItem = async function(table, idx) {
    const dataArray = table === 'products' ? dbProducts : (table === 'services' ? dbServices : dbPortfolio);
    if(confirm("Удалить запись?")) { try { await mutateDB(table, 'DELETE', null, dataArray[idx].id); await initApp(); } catch(err) { alert(err.message); } }
}

window.editItem = function(table, i) {
    const item = (table === 'products' ? dbProducts : (table === 'services' ? dbServices : dbPortfolio))[i];
    editIds[table] = item.id;
    if (table === 'products') {
        document.getElementById('admin-title').value = item.title; document.getElementById('admin-price').value = item.price; document.getElementById('admin-subtitle').value = item.subtitle || ''; document.getElementById('admin-desc').value = item.desc_text || ''; document.getElementById('admin-quantity').value = parseInt(item.specs_card?.[0]?.value) || 0; 
        loadImg(item.image,'product-preview','admin-image','product-drop-text');
        ['category','stock','brand','power','efficiency','frame','phase','voltage','capacity'].forEach(k=>setCustomSelectValue('admin-'+k, item[k]||'all'));
        setCustomSelectValue('admin-cell-type', item.cell||'all');
    } else if (table === 'services') {
        document.getElementById('service-title').value = item.title; document.getElementById('service-price').value = item.price; document.getElementById('service-desc').value = item.desc_text;
    } else if (table === 'portfolio') {
        document.getElementById('port-title').value = item.title; document.getElementById('port-savings').value = item.savings || ''; document.getElementById('port-price').value = item.price || ''; document.getElementById('port-equipment').value = item.equipment || ''; document.getElementById('port-desc').value = item.desc_text || ''; 
        loadImg(item.images,'port-preview','port-image','port-drop-text');
    }
...
const cancelEdit = (table) => {
    editIds[table] = null; document.getElementById(`admin-${table === 'products' ? 'product' : table}-form`).reset();
    ...
    document.getElementById(`btn-submit-${table}`).innerText = table === 'portfolio' ? '+ Опубликовать' : '+ Сохранить'; document.getElementById(`btn-cancel-${table}`).classList.add('hidden');
}
...
if (document.getElementById('admin-product-form')) {
    document.getElementById('admin-product-form').onsubmit = (e) => handleAdminSubmit(e, 'products', () => {
        const getV = id => document.getElementById(id)?.getAttribute('data-value') || 'all'; const cat = getV('admin-category'); const qty = parseInt(document.getElementById('admin-quantity').value) || 0;
        return {
          ...
          specs_card: [{label:"Доступно:",value: qty + " шт."}], specs_full: [{label:"В наличии:",value: qty + " шт."}]
        };
    });
...
function initAdminSettings() {
    ...
    if (document.getElementById('admin-telegram-form')) {
        ...
        document.getElementById('tg-add-btn').onclick = async () => { 
            ...
            if(!chatIds.find(c=>c.id===i)){ try { chatIds.push({id:i,label:l}); await mutateDB('settings', 'PATCH', { tg_chat_ids: JSON.stringify(chatIds) }, 1); document.getElementById('tg-new-label').value=''; document.getElementById('tg-new-chatid').value=''; await initApp(); } catch (err) { alert(err.message); } } 
        };
...
function renderContacts() {
    ...
    if (footerPhone) { if (contacts[0]?.trim()) { footerPhone.innerText = contacts[0]; footerPhone.href = `tel:${contacts[0].replace(/[^\d+]/g, '')}`; footerPhone.style.display = 'block'; } else { footerPhone.style.display = 'none'; } }
}
...
function applyAdvancedFilters() {
    ...
    cards.forEach(card => {
        ...
        let ok = (f.c==='all'||d.category===f.c) && (f.s==='all'||d.stock===f.s) && (f.b==='all'||d.brand===f.b);
        if(f.c==='panels') ok = ok && (f.pw==='all'||d.power===f.pw) && (f.cl==='all'||d.cell===f.cl) && (f.ef==='all'||d.efficiency===f.ef) && (f.fr==='all'||d.frame===f.fr);
        else if(f.c==='batteries') ok = ok && (f.cp==='all'||d.capacity===f.cp) && (f.v==='all'||d.voltage===f.v);
        else if(f.c==='inverters') ok = ok && (f.ph==='all'||d.phase===f.ph) && (f.v==='all'||d.voltage===f.v);
...
function renderServices() {
  const sGrid = document.getElementById('services-grid'); if (!sGrid) return;
...
function renderPortfolio() {
  const pg = document.getElementById('portfolio-grid'); if(!pg) return;
  pg.innerHTML = dbPortfolio.map((p,i) => {
    let img = p.images?`<img src="${p.images}" class="port-img" loading="lazy">`:`<div class="mock-img port-img">📸</div>`;
    return `<div class="portfolio-card card-base card-item" data-id="${i}">${img}<div class="port-content"><h3 class="port-title">${p.title}</h3><p class="port-desc">${p.desc_text}</p><button class="btn btn-sm btn-drawer-port mt-auto btn-full">Посмотреть детали</button></div></div>`;
  }).join('');
}

// =========================================================
// 🚀 ИНТЕРАКТИВ И УНИВЕРСАЛЬНАЯ ШТОРКА (DRAWER)
// =========================================================
function openDrawer(title, desc, price, imgHtml, specsHtml) {
    document.getElementById('u-title').innerText = title;
    document.getElementById('u-desc').innerText = desc;
    document.getElementById('u-price').innerText = price;
    document.getElementById('u-img').innerHTML = imgHtml;
    document.getElementById('u-specs').innerHTML = specsHtml;
    document.getElementById('u-drawer-overlay')?.classList.add('active'); 
    document.getElementById('u-drawer')?.classList.add('active'); 
...
document.addEventListener('click', (e) => {
  ...
  if (btnLead) {
    ...
    document.getElementById('lead-modal')?.classList.add('active'); document.body.style.overflow = 'hidden';
  }

  if (e.target.closest('.btn-drawer')) {
    const p = dbProducts[e.target.closest('.tech-card').dataset.id];
    if(p) {
      const img = p.image ? `<img src="${p.image}" loading="lazy">` : '📸';
      const specs = p.specs_full ? p.specs_full.map(s=>`<li><strong>${s.label}</strong> ${s.value}</li>`).join('') : '';
      openDrawer(p.title, p.desc_text, p.price, img, specs);
    }
  }
  
  if (e.target.closest('.btn-drawer-port')) {
    const p = dbPortfolio[e.target.closest('.portfolio-card').dataset.id];
    if(p) {
      const img = p.images ? `<img src="${p.images}" loading="lazy">` : '📸';
      ...
      openDrawer(p.title, p.desc_text, p.price || '-', img, sHtml);
    }
  }
});
  ...
  tgForm.addEventListener('submit', function(e) {
    ...
    Promise.all(requests).then(responses => {
        if(responses.every(res => res.ok)) {
            document.getElementById('lead-success').classList.remove('hidden'); tgForm.reset();
        ...
        } else { alert("Ошибка отправки. Проверьте настройки в админке."); btn.innerText = "Отправить заявку"; btn.disabled = false; }
      }).catch(() => { alert("Сетевая ошибка."); btn.innerText = "Отправить заявку"; btn.disabled = false; });
  });
}

// =========================================================
// 🧮 КАЛЬКУЛЯТОР (КЭШИРОВАНИЕ DOM)
// =========================================================
const rs = document.getElementById('roofArea'), rn = document.getElementById('roofAreaNum');
const bs = document.getElementById('monthlyBill'), bn = document.getElementById('monthlyBillNum');
const pRes = document.getElementById('powerRes'), sRes = document.getElementById('savingsRes'), pbRes = document.getElementById('paybackRes');

function calc() { 
    if(!rs || !bs) return;
    const r = Number(rs.value), b = Number(bs.value); 
    if(rn) rn.value = r; if(bn) bn.value = b; 
    const m = (r * 0.16).toFixed(1); const s = Math.round(b * 12 * 0.8);
    if(pRes) pRes.innerText = `${m} кВт`; 
    if(sRes) sRes.innerText = `~${s.toLocaleString('ru-RU')} ₽`; 
    if(pbRes) pbRes.innerText = `~${((m * 85000) / s).toFixed(1)} года`; 
}
